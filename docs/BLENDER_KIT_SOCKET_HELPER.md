...
- create an empty at the object's origin with the same rotation and identity
  scale, or preserve the source object's full transform when enabled;
...
3. In `Bionicle Kit`:
   ...
   - enable `Preserve Source Transform` only when the socket should also keep
     the source object's scale;
