...
Part of the broader draw-call plan in [`3D_RENDERING_STRATEGY.md`](3D_RENDERING_STRATEGY.md). Phase B ships **before** battle LOD (Phase C) because it is self-contained, testable, and shaves draws on every kit-attached character without re-exporting GLBs.
...
~10% draw reduction; triangles and materials unchanged. Battle LOD (Phase C) is required for the ≤ 50 **total** battle budget.
