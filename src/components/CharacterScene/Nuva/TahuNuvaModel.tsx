...
} from '../../../game/kit/attachments/Toa Nuva/tahu';
import type { WeatheredMetalOptions } from '../WeatheredMetalMaterial';

const TAHU_NUVA_WEATHERED: WeatheredMetalOptions = {
  cavityStrength: 1,
  edgeColor: '#ffffff',
...
const TAHU_NUVA_KIT_ATTACHMENT_RUNS = 2;
...
>(({ matoran, onKitMeshesAttached }, ref) => {
  ...
  const { animations, nodes } = useGLTF(import.meta.env.BASE_URL + 'Toa_Nuva/tahu.glb');
  ...
  const onKitLayerAttached = useMemo(() => {
    ...
    return () => {
      ...
      if (kitLayersDone.current >= TAHU_NUVA_KIT_ATTACHMENT_RUNS) {
  ...
  useKitAttachments({
    attachments: TAHU_NUVA_KIT_2001_ATTACHMENTS,
    characterNodes,
  ...
  useKitAttachments({
    attachments: TAHU_NUVA_KIT_2003_ATTACHMENTS,
    characterNodes,
    colors: matoran.colors,
    kitSlotMap: matoran.kitSlotMap,
