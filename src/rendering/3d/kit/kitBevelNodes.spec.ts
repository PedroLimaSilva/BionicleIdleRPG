...
describe('kit bevel allowlists', () => {
  test('every listed 2001 node is a registered kit constant', () => {
    const registered = new Set(Object.values(KIT_2001_NODES));
    for (const name of KIT_2001_BEVEL_NODES) {
      expect(registered.has(name)).toBe(true);
    }
  });

  test('every listed 2003 node is a registered kit constant', () => {
    const registered = new Set(Object.values(KIT_2003_NODES));
    for (const name of KIT_2003_BEVEL_NODES) {
      expect(registered.has(name)).toBe(true);
    }
  });

  test('every listed 2004 node is a registered kit constant', () => {
    const registered = new Set(Object.values(KIT_2004_NODES));
    for (const name of KIT_2004_BEVEL_NODES) {
      expect(registered.has(name)).toBe(true);
    }
  });

  test('low-detail 2001 connectors are not on the bevel allowlist', () => {
    const listed = new Set(KIT_2001_BEVEL_NODES);
    for (const name of KIT_2001_BEVEL_SKIP_CONNECTORS) {
      expect(listed.has(name)).toBe(false);
    }
  });

  test('undeclared nodes and unknown kits have no map', () => {
  ...
  test('filter keeps unique declared names and drops the rest', () => {
    const listed = new Set<string>(KIT_2001_BEVEL_NODES);
    const mixed = [
    ...
    expect(filtered.every((name) => listed.has(name))).toBe(true);
